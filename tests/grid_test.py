...
class TestGetNeighbours:
    ...
    def test_raises_error_if_out_of_range(self, grid_5_7: Grid):
        ...
        with pytest.raises(AssertionError):
            grid_5_7.get_neighbours(-1, 0)
        with pytest.raises(AssertionError):
            grid_5_7.get_neighbours(0, -1)
        with pytest.raises(AssertionError):
            grid_5_7.get_neighbours(-4, -4)

        # Equal to grid size
        with pytest.raises(AssertionError):
            grid_5_7.get_neighbours(grid_5_7.dim.x, 0)
        with pytest.raises(AssertionError):
            grid_5_7.get_neighbours(0, grid_5_7.dim.y)
        with pytest.raises(AssertionError):
            grid_5_7.get_neighbours(grid_5_7.dim.x, grid_5_7.dim.y)

        # More than grid size
        with pytest.raises(AssertionError):
            grid_5_7.get_neighbours(grid_5_7.dim.x + 1, 0)
        with pytest.raises(AssertionError):
            grid_5_7.get_neighbours(0, grid_5_7.dim.y + 1)
        with pytest.raises(AssertionError):
            grid_5_7.get_neighbours(grid_5_7.dim.x + 4, grid_5_7.dim.y + 4)

    def test_for_corners(self, grid_5_7: Grid):
        # Top left
        self.check_list_of_cells(
            grid_5_7.get_neighbours(0, 0),
            [(0, 1), (1, 0), (1, 1)]
        )

        # Bottom left
        self.check_list_of_cells(
            grid_5_7.get_neighbours(0, 6),
            [(0, 5), (1, 6), (1, 5)]
        )

        # Top right
        self.check_list_of_cells(
            grid_5_7.get_neighbours(4, 0),
            [(3, 0), (4, 1), (3, 1)]
        )

        # Bottom right
        self.check_list_of_cells(
            grid_5_7.get_neighbours(4, 6),
            [(3, 6), (4, 5), (3, 6)]
        )
    ...
    def test_for_sides(self, grid_5_7: Grid):
        # Top
        self.check_list_of_cells(
            grid_5_7.get_neighbours(2, 0),
            [(1, 0), (1, 1), (2, 1), (3, 0), (3, 1)]
        )

        # Left
        self.check_list_of_cells(
            grid_5_7.get_neighbours(0, 2),
            [(0, 1), (0, 3), (1, 1), (1, 2), (1, 3)]
        )

        # Bottom
        self.check_list_of_cells(
            grid_5_7.get_neighbours(2, 6),
            [(1, 5), (1, 6), (2, 5), (3, 5), (3, 6)]
        )

        # Right
        self.check_list_of_cells(
            grid_5_7.get_neighbours(4, 2),
            [(3, 1), (3, 2), (3, 3), (4, 1), (4, 3)]
        )
    ...
    def test_all_other_cells(self, grid_5_7: Grid):
        for pos_x in range(1, grid_5_7.dim.x - 1):
            for pos_y in range(1, grid_5_7.dim.y - 1):
                self.check_list_of_cells(
                    grid_5_7.get_neighbours(pos_x, pos_y),
                    [
